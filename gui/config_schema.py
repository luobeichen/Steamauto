...
CONFIG_SCHEMA = [
    ...
    # ============ 悠悠有品 自动发货 ============
    {"group": "悠悠有品", "label": "启用", "key": "uu.accept_offer.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品自动发货功能是否启用。"},
    {"group": "悠悠有品", "label": "轮询间隔", "key": "uu.accept_offer.interval", "type": "int",
     "default": 300, "options": "整数，单位：秒", "help": "每次检查新报价的间隔（秒）。"},

    # ============ 悠悠有品 租赁自动上架 ============
    {"group": "悠悠有品 · 出租", "label": "启用", "key": "uu.lease_item.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品租赁自动上架功能是否启用。"},
    {"group": "悠悠有品 · 出租", "label": "最长租赁天数", "key": "uu.lease_item.lease_max_days", "type": "int",
     "default": 60, "options": "整数，单位：天", "help": "最长租赁时间（天）。"},
    {"group": "悠悠有品 · 出租", "label": "最低上架价格", "key": "uu.lease_item.filter_price", "type": "int",
     "default": 100, "options": "整数，单位：元", "help": "价格低于此值的物品不会上架。"},
    {"group": "悠悠有品 · 出租", "label": "定时运行时间", "key": "uu.lease_item.run_time", "type": "string",
     "default": "17:30", "options": "HH:MM 格式", "help": "自动上架租赁每天定时运行时间。"},
    {"group": "悠悠有品 · 出租", "label": "改价间隔", "key": "uu.lease_item.interval", "type": "int",
     "default": 31, "options": "整数，单位：分钟", "help": "已上架租赁物品定时改价的轮询间隔（分钟）。"},
    {"group": "悠悠有品 · 出租", "label": "不出租物品", "key": "uu.lease_item.filter_name", "type": "array",
     "default": [], "options": "字符串数组", "help": "不出租的物品名字列表（名字可不写全，但要写对）。"},
    {"group": "悠悠有品 · 出租", "label": "固定比例定价", "key": "uu.lease_item.enable_fix_lease_ratio", "type": "bool",
     "default": False, "options": "true / false", "help": "是否按现价固定比例设置出租价格。"},
    {"group": "悠悠有品 · 出租", "label": "出租价格比例", "key": "uu.lease_item.fix_lease_ratio", "type": "float",
     "default": 0.001, "options": "小数，如 0.001", "help": "出租价格比例。如现价 1000 元、比例 0.001 → 1 元。"},
    {"group": "悠悠有品 · 出租", "label": "赔付方式", "key": "uu.lease_item.compensation_type", "type": "int",
     "default": 7, "options": "0(非会员) / 7(v1)", "help": "赔付方式：0 表示非会员，7 表示 v1。"},

    # ============ 悠悠有品 出售自动上架 ============
    {"group": "悠悠有品 · 出售", "label": "启用", "key": "uu.sell_item.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品出售自动上架功能是否启用。"},
    {"group": "悠悠有品 · 出售", "label": "止盈定价", "key": "uu.sell_item.take_profile", "type": "bool",
     "default": False, "options": "true / false", "help": "是否按照止盈率设置定价。"},
    {"group": "悠悠有品 · 出售", "label": "止盈率", "key": "uu.sell_item.take_profile_ratio", "type": "float",
     "default": 0.1, "options": "小数，如 0.1", "help": "止盈率（配合止盈定价使用）。"},
    {"group": "悠悠有品 · 出售", "label": "定时运行时间", "key": "uu.sell_item.run_time", "type": "string",
     "default": "15:30", "options": "HH:MM 格式", "help": "自动上架每天定时运行时间。"},
    {"group": "悠悠有品 · 出售", "label": "改价请求间隔", "key": "uu.sell_item.sell_interval", "type": "int",
     "default": 20, "options": "整数，单位：分钟", "help": "每隔多久重新请求市场并改价一次（分钟）。"},
    {"group": "悠悠有品 · 出售", "label": "最高上架价格", "key": "uu.sell_item.max_on_sale_price", "type": "int",
     "default": 1000, "options": "整数，0 不限制", "help": "价格高于此值的物品不会上架，设为 0 则不限制。"},
    {"group": "悠悠有品 · 出售", "label": "改价轮询间隔", "key": "uu.sell_item.interval", "type": "int",
     "default": 51, "options": "整数，单位：分钟", "help": "已上架物品定时改价的轮询间隔（分钟）。"},
    {"group": "悠悠有品 · 出售", "label": "出售物品", "key": "uu.sell_item.name", "type": "array",
     "default": [], "options": "字符串数组", "help": "出售的物品名字列表（名字可不写全，但要写对）。"},
    {"group": "悠悠有品 · 出售", "label": "排除物品", "key": "uu.sell_item.blacklist_words", "type": "array",
     "default": [], "options": "字符串数组", "help": "不出售也不参与改价的物品名字列表（优先级高于出售列表）。"},
    {"group": "悠悠有品 · 出售", "label": "自动压价", "key": "uu.sell_item.use_price_adjustment", "type": "bool",
     "default": True, "options": "true / false", "help": "是否开启自动压价（-0.01）功能。"},
    {"group": "悠悠有品 · 出售", "label": "压价阈值", "key": "uu.sell_item.price_adjustment_threshold", "type": "float",
