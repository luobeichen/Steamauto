...
CONFIG_SCHEMA = [
    # ============ Steam 基础 ============
    {"group": "Steam", "label": "关闭 SSL 验证", "key": "steam_login_ignore_ssl_error", "type": "bool",
     "default": False, "options": "true / false", "help": "登录 Steam 时是否关闭 SSL 证书验证。正常情况下不建议关闭。"},
    {"group": "Steam", "label": "本地加速", "key": "steam_local_accelerate", "type": "bool",
     "default": False, "options": "true / false", "help": "是否开启本地加速（非 100% 可用）。开启此功能必须同时关闭 SSL 验证。"},
    {"group": "Steam", "label": "手动指定代理", "key": "use_proxies", "type": "bool",
     "default": False, "options": "true / false", "help": "是否手动指定 Steam 代理（该功能只代理 Steam，需配合下方 proxies）。"},
    {"group": "Steam", "label": "HTTP 代理地址", "key": "proxies.http", "type": "string",
     "default": "http://127.0.0.1:7890", "options": "URL 字符串", "help": "HTTP 代理地址，仅当 use_proxies 为 true 时生效。"},
    {"group": "Steam", "label": "HTTPS 代理地址", "key": "proxies.https", "type": "string",
     "default": "http://127.0.0.1:7890", "options": "URL 字符串", "help": "HTTPS 代理地址，一般与 HTTP 相同。"},

    # ============ 通知服务 ============
    {"group": "通用配置", "label": "通知器列表", "key": "notify_service.notifiers", "type": "array",
     "default": [], "options": "Apprise 格式地址数组", "help": "通知服务器列表（Apprise 格式），支持 Telegram/钉钉/飞书/WxPusher/Server酱等；为空不发送。"},
    {"group": "通用配置", "label": "自定义标题", "key": "notify_service.custom_title", "type": "string",
     "default": "", "options": "字符串，留空用默认", "help": "自定义通知标题，为空则使用默认标题。"},
    {"group": "通用配置", "label": "包含 Steam 账号信息", "key": "notify_service.include_steam_info", "type": "bool",
     "default": True, "options": "true / false", "help": "通知中是否包含 Steam 账号信息。"},
    {"group": "通用配置", "label": "屏蔽词", "key": "notify_service.blacklist_words", "type": "array",
     "default": [], "options": "字符串数组", "help": "通知内容包含这些词时不会发送。"},

    # ============ BUFF 自动发货 ============
    {"group": "BUFF", "label": "启用", "key": "buff_auto_accept_offer.enable", "type": "bool",
     "default": True, "options": "true / false", "help": "是否启用 BUFF 自动发货报价功能。"},
    {"group": "BUFF", "label": "轮询间隔", "key": "buff_auto_accept_offer.interval", "type": "int",
     "default": 300, "options": "整数，单位：秒", "help": "每次检查是否有新报价的间隔（秒）。"},
    {"group": "BUFF", "label": "dota2 支持", "key": "buff_auto_accept_offer.dota2_support", "type": "bool",
     "default": False, "options": "true / false", "help": "是否开启 dota2 支持。"},
    {"group": "BUFF", "label": "使用代理", "key": "buff_auto_accept_offer.use_proxies", "type": "bool",
     "default": False, "options": "true / false", "help": "是否使用全局代理设置中的代理连接 BUFF。"},

    # ============ 悠悠有品 自动发货 ============
    {"group": "悠悠有品", "label": "启用", "key": "uu_auto_accept_offer.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品自动发货功能是否启用。"},
    {"group": "悠悠有品", "label": "轮询间隔", "key": "uu_auto_accept_offer.interval", "type": "int",
     "default": 300, "options": "整数，单位：秒", "help": "每次检查新报价的间隔（秒）。"},
    {"group": "悠悠有品", "label": "使用代理", "key": "uu_auto_accept_offer.use_proxies", "type": "bool",
     "default": False, "options": "true / false", "help": "是否使用全局代理连接悠悠有品。"},

    # ============ 悠悠有品 租赁自动上架 ============
    {"group": "悠悠有品 · 出租", "label": "启用", "key": "uu_auto_lease_item.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品租赁自动上架功能是否启用。"},
    {"group": "悠悠有品 · 出租", "label": "最长租赁天数", "key": "uu_auto_lease_item.lease_max_days", "type": "int",
     "default": 60, "options": "整数，单位：天", "help": "最长租赁时间（天）。"},
    {"group": "悠悠有品 · 出租", "label": "最低上架价格", "key": "uu_auto_lease_item.filter_price", "type": "int",
     "default": 100, "options": "整数，单位：元", "help": "价格低于此值的物品不会上架。"},
    {"group": "悠悠有品 · 出租", "label": "定时运行时间", "key": "uu_auto_lease_item.run_time", "type": "string",
     "default": "17:30", "options": "HH:MM 格式", "help": "自动上架租赁每天定时运行时间。"},
    {"group": "悠悠有品 · 出租", "label": "改价间隔", "key": "uu_auto_lease_item.interval", "type": "int",
     "default": 31, "options": "整数，单位：分钟", "help": "已上架租赁物品定时改价的轮询间隔（分钟）。"},
    {"group": "悠悠有品 · 出租", "label": "不出租物品", "key": "uu_auto_lease_item.filter_name", "type": "array",
     "default": [], "options": "字符串数组", "help": "不出租的物品名字列表（名字可不写全，但要写对）。"},
    {"group": "悠悠有品 · 出租", "label": "固定比例定价", "key": "uu_auto_lease_item.enable_fix_lease_ratio", "type": "bool",
     "default": False, "options": "true / false", "help": "是否按现价固定比例设置出租价格。"},
    {"group": "悠悠有品 · 出租", "label": "出租价格比例", "key": "uu_auto_lease_item.fix_lease_ratio", "type": "float",
     "default": 0.001, "options": "小数，如 0.001", "help": "出租价格比例。如现价 1000 元、比例 0.001 → 1 元。"},
    {"group": "悠悠有品 · 出租", "label": "赔付方式", "key": "uu_auto_lease_item.compensation_type", "type": "int",
     "default": 7, "options": "0(非会员) / 7(v1)", "help": "赔付方式：0 表示非会员，7 表示 v1。"},

    # ============ 悠悠有品 出售自动上架 ============
    {"group": "悠悠有品 · 出售", "label": "启用", "key": "uu_auto_sell_item.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "悠悠有品出售自动上架功能是否启用。"},
    {"group": "悠悠有品 · 出售", "label": "止盈定价", "key": "uu_auto_sell_item.take_profile", "type": "bool",
     "default": False, "options": "true / false", "help": "是否按照止盈率设置定价。"},
    {"group": "悠悠有品 · 出售", "label": "止盈率", "key": "uu_auto_sell_item.take_profile_ratio", "type": "float",
     "default": 0.1, "options": "小数，如 0.1", "help": "止盈率（配合止盈定价使用）。"},
    {"group": "悠悠有品 · 出售", "label": "定时运行时间", "key": "uu_auto_sell_item.run_time", "type": "string",
     "default": "15:30", "options": "HH:MM 格式", "help": "自动上架每天定时运行时间。"},
    {"group": "悠悠有品 · 出售", "label": "改价请求间隔", "key": "uu_auto_sell_item.sell_interval", "type": "int",
     "default": 20, "options": "整数，单位：分钟", "help": "每隔多久重新请求市场并改价一次（分钟）。"},
    {"group": "悠悠有品 · 出售", "label": "最高上架价格", "key": "uu_auto_sell_item.max_on_sale_price", "type": "int",
     "default": 1000, "options": "整数，0 不限制", "help": "价格高于此值的物品不会上架，设为 0 则不限制。"},
    {"group": "悠悠有品 · 出售", "label": "改价轮询间隔", "key": "uu_auto_sell_item.interval", "type": "int",
     "default": 51, "options": "整数，单位：分钟", "help": "已上架物品定时改价的轮询间隔（分钟）。"},
    {"group": "悠悠有品 · 出售", "label": "出售物品", "key": "uu_auto_sell_item.name", "type": "array",
     "default": [], "options": "字符串数组", "help": "出售的物品名字列表（名字可不写全，但要写对）。"},
    {"group": "悠悠有品 · 出售", "label": "排除物品", "key": "uu_auto_sell_item.blacklist_words", "type": "array",
     "default": [], "options": "字符串数组", "help": "不出售也不参与改价的物品名字列表（优先级高于出售列表）。"},
    {"group": "悠悠有品 · 出售", "label": "自动压价", "key": "uu_auto_sell_item.use_price_adjustment", "type": "bool",
     "default": True, "options": "true / false", "help": "是否开启自动压价（-0.01）功能。"},
    {"group": "悠悠有品 · 出售", "label": "压价阈值", "key": "uu_auto_sell_item.price_adjustment_threshold", "type": "float",
     "default": 1.0, "options": "小数，单位：元", "help": "价格高于此值才会自动压价。"},

    # ============ Steam 自动收礼 ============
    {"group": "Steam", "label": "启用", "key": "steam_auto_accept_offer.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "是否自动接受 Steam 礼物报价（无需支出库存物品的报价）。"},
    {"group": "Steam", "label": "轮询间隔", "key": "steam_auto_accept_offer.interval", "type": "int",
    ...
    {"group": "ECOSteam · 出售同步", "label": "同步出售启用", "key": "ecosteam.auto_sync_sell_shelf.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "是否自动同步各平台的上架商品（与主平台一致）。"},
    {"group": "ECOSteam · 出售同步", "label": "同步出售主平台", "key": "ecosteam.auto_sync_sell_shelf.main_platform", "type": "string",
     "default": "eco", "options": "buff / uu / eco", "help": "主平台（不会被改动），按价格比例同步到其他平台。"},
    {"group": "ECOSteam · 出售同步", "label": "同步出售启用平台", "key": "ecosteam.auto_sync_sell_shelf.enabled_platforms", "type": "array",
     "default": ["uu"], "options": "数组，可选 buff/uu", "help": "可填入多个平台，如 [\"buff\",\"uu\"]。ECO 已强制开启。"},
    {"group": "ECOSteam · 出售同步", "label": "同步出售比例 eco", "key": "ecosteam.auto_sync_sell_shelf.ratio.eco", "type": "float",
     "default": 1, "options": "小数", "help": "ECO 平台上架价格比例。"},
    {"group": "ECOSteam · 出售同步", "label": "同步出售比例 uu", "key": "ecosteam.auto_sync_sell_shelf.ratio.uu", "type": "float",
     "default": 1, "options": "小数", "help": "悠悠有品平台上架价格比例。"},
    {"group": "ECOSteam · 出售同步", "label": "同步出售比例 buff", "key": "ecosteam.auto_sync_sell_shelf.ratio.buff", "type": "float",
     "default": 1, "options": "小数", "help": "BUFF 平台上架价格比例。"},
    {"group": "ECOSteam · 出租同步", "label": "同步租赁启用", "key": "ecosteam.auto_sync_lease_shelf.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "是否与悠悠有品平台同步租赁商品。"},
    {"group": "ECOSteam · 出租同步", "label": "同步租赁主平台", "key": "ecosteam.auto_sync_lease_shelf.main_platform", "type": "string",
     "default": "eco", "options": "uu / eco", "help": "租赁同步主平台（不会被改动）。"},
    {"group": "ECOSteam · 出租同步", "label": "同步租赁比例 eco", "key": "ecosteam.auto_sync_lease_shelf.ratio.eco", "type": "float",
     "default": 1, "options": "小数", "help": "ECO 平台租赁价格比例。"},
    {"group": "ECOSteam · 出租同步", "label": "同步租赁比例 uu", "key": "ecosteam.auto_sync_lease_shelf.ratio.uu", "type": "float",
    ...
    # ============ C5 自动发货 ============
    {"group": "C5", "label": "启用", "key": "c5_auto_accept_offer.enable", "type": "bool",
     "default": False, "options": "true / false", "help": "C5 自动发货功能是否启用。"},
    {"group": "C5", "label": "轮询间隔", "key": "c5_auto_accept_offer.interval", "type": "int",
     "default": 30, "options": "整数，单位：秒", "help": "每次检查新报价的间隔（秒）。"},
    {"group": "C5", "label": "AppKey", "key": "c5_auto_accept_offer.app_key", "type": "string",
     "default": "", "options": "字符串", "help": "C5Game 的 AppKey，在 c5game.com 开放平台申请。"},

    # ============ 日志与通用 ============
    {"group": "通用配置", "label": "日志等级", "key": "log_level", "type": "string",
     "default": "info", "options": "debug / info / warning / error", "help": "写入硬盘的日志等级。"},
    {"group": "通用配置", "label": "日志保留天数", "key": "log_retention_days", "type": "int",
     "default": 7, "options": "整数，单位：天", "help": "本地日志保留天数，超过自动删除。"},
    {"group": "通用配置", "label": "出错即停", "key": "no_pause", "type": "bool",
     "default": False, "options": "true / false", "help": "设为 true 后，程序出现错误直接停止（不再等待按键）。不确定请保持 false。"},
    {"group": "通用配置", "label": "发货人工确认", "key": "manual_confirm_delivery", "type": "bool",
     "default": True, "options": "true / false", "help": "检测到待发货报价时仅发送通知、不自动接受，由人工在平台上确认发货。"},
    {"group": "通用配置", "label": "插件白名单", "key": "plugin_whitelist", "type": "array",
     "default": [], "options": "字符串数组", "help": "白名单内的本地插件与程序自带版本不同时不会被覆盖。"},
    {"group": "通用配置", "label": "源码自动更新", "key": "source_code_auto_update", "type": "bool",
